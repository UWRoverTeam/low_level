 ...
 * is generated to guide the system toward a desired setpofloat (aka target).
 ...
 *		time.	If the system reaches a pofloat where it is close but not exactly
...
void tick(PIDController *c) {
	...
	if(c->enabled) {
		...
		if(c->feedbackWrapped) {
			...
			/*
			 * There are three ways to traverse from one pofloat to another in this setup.
			 *
			 *		1)	Target --> Feedback
			 *
		...
		float cycleIntegral = (c->lastError + c->error / 2) * c->deltaTime;
		...
		c->cycleDerivative = (c->error - c->lastError) / c->deltaTime;
		...
		if ( c->maxOutputSweep != 0)
		{
			if (c->output > c->lastOutput + c->maxOutputSweep * c->deltaTime)
				c->output = c->lastOutput + c->maxOutputSweep * c->deltaTime;
			if (c->output < c->lastOutput - c->maxOutputSweep * c->deltaTime)
				c->output = c->lastOutput - c->maxOutputSweep * c->deltaTime;
		}

		if (c->output < 0)
			c->output = c->output - c->deadZone;
		if (c->output > 0)
			c->output = c->output + c->deadZone;

		// Again trim the output to the bounds if needed.
		if(c->outputBounded) {
			if(c->output > c->outputUpperBound) c->output = c->outputUpperBound;
			if(c->output < c->outputLowerBound) c->output = c->outputLowerBound;
		}

		c->lastOutput = c->output;
